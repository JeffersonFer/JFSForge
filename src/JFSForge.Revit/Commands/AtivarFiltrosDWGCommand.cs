...
namespace JFSForge.Revit.Commands
{
    ...
    internal class AtivarFiltrosDWGCommand : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            int filtrosAtivados = 0;
            int templatesAfetados = 0;
            var templatesNaoControlados = new List<string>();

            using (var trans = new Transaction(doc, "Ativar Filtros DWG nos Templates"))
            {
                trans.Start();
                ...
                foreach (var template in templates)
                {
                    ...
                    bool ativouNesteTemplate = false;
                    ...
                    foreach (var filtroId in template.GetFilters())
                    {
                        ...
                        template.SetIsFilterEnabled(filtroId, true);
                        filtrosAtivados++;
                        ativouNesteTemplate = true;
                    }

                    if (ativouNesteTemplate)
                        templatesAfetados++;
                }
            ...
            resumo.AppendLine($"Filtros DWG ativados: {filtrosAtivados}");
            ...
            TaskDialog.Show("Ativar Filtros DWG", resumo.ToString());
